...
# --- FUENTES ---
FEEDS_SPAIN = ["https://elpais.com/rss/politica/portada.xml", "https://www.elmundo.es/e/rss/espana.xml"]
FEEDS_ARGENTINA = ["https://www.lanacion.com.ar/arc/outboundfeeds/rss/?outputType=xml", "https://www.clarin.com/rss/politica/"]
FEEDS_BRASIL = ["https://feeds.folha.uol.com.br/poder/rss091.xml", "https://agenciabrasil.ebc.com.br/rss/politica/feed.xml"]
...
def ejecutar():
    ...
    cur = conn.cursor()
    cur.execute('''CREATE TABLE IF NOT EXISTS news (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, region TEXT, title TEXT, link TEXT UNIQUE, sentimiento REAL)''')
    cur.execute("DELETE FROM news WHERE timestamp < datetime('now', '-7 days')")
    
    ahora = datetime.now()
    fecha_str = ahora.strftime('%Y-%m-%d %H:%M')
    print("--- INICIANDO CAPTURA MULTI-REGIONAL ---")
    
    total_articulos = 0
    for reg, info in DATOS_INTEL.items():
        if info["url"] == "COMBO": feeds_a_procesar = FEEDS_SPAIN
        elif info["url"] == "COMBO_ARG": feeds_a_procesar = FEEDS_ARGENTINA
        elif info["url"] == "COMBO_BR": feeds_a_procesar = FEEDS_BRASIL
        else: feeds_a_procesar = [info["url"]]
        ...
        for url in feeds_a_procesar:
            ...
            for e in f_parse.entries[:10]:
                ...
                cur.execute("INSERT OR IGNORE INTO news (region, title, link, sentimiento) VALUES (?, ?, ?, ?)", (reg, e.title, e.link, pola))
                total_articulos += 1
    conn.commit()
    ...
    with open(os.path.join(POSTS_OUTPUT, filename), 'w') as f:
        ...
        for reg, tit, link in cur.fetchall():
            ...
            tit_l = tit.lower()
            if any(key in tit_l for key in KEYWORDS_CRITICAS): alertas.append(txt.replace("**[", "🚩 **[ALERTA] "))
            elif any(key in tit_l for key in KEYWORDS_ELECTORALES): electoral.append(txt.replace("**[", "🗳️ **[ELECTORAL] "))
            else: normales.append(txt)

        if alertas: f.write(f"### ⚡ ALERTAS CRÍTICAS\n\n" + "\n".join(alertas) + "\n\n")
